...
interface FiscalAgentInviteOptions {
  to: string;
  orgName: string;
  signupUrl: string;
}

// The "signup link" promised on the pay-first checkout return page. Carries the
// one-time invite token so the charity can claim their account and finish the
// draft listing. Same receipt-style shell as the other transactional emails.
export async function sendFiscalAgentInviteEmail(opts: FiscalAgentInviteOptions): Promise<void> {
  const orgName = escapeHtml(opts.orgName);
  const signupUrl = encodeURI(opts.signupUrl);
...
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width,initial-scale=1.0">
  <title>Finish setting up your listing — GrantTrail</title>
</head>
<body style="margin:0;padding:0;background-color:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color:#f3f4f6;padding:40px 16px;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" role="presentation" style="background-color:#ffffff;border-radius:8px;overflow:hidden;max-width:600px;width:100%;">
          ...
          <tr>
            <td style="background-color:#1a6b3c;padding:32px 40px;text-align:center;">
              <p style="margin:0;font-size:26px;font-weight:700;color:#ffffff;letter-spacing:-0.5px;">GrantTrail</p>
              <p style="margin:8px 0 0;font-size:14px;color:rgba(255,255,255,0.75);">Finish Setting Up Your Listing</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:40px;">
              ...
              <p style="margin:0 0 20px;font-size:16px;color:#111111;">Hi ${orgName},</p>
              <p style="margin:0 0 32px;font-size:16px;color:#444444;line-height:1.6;">
                Thanks for your payment. Your Fiscal Agent listing has been created as a draft.
                Use the link below to claim your account and complete it — it will appear in the
                directory once you finish and we verify your 501(c)(3) status.
              </p>
              ...
              <p style="margin:0;font-size:14px;color:#6b7280;line-height:1.6;">
                Can't use the button? Open this link:<br>
                <a href="${signupUrl}" style="color:#1a6b3c;text-decoration:none;word-break:break-all;">${signupUrl}</a>
              </p>
          ...
          <tr>
            <td style="background-color:#f9fafb;padding:24px 40px;border-top:1px solid #e5e7eb;text-align:center;">
              <p style="margin:0;font-size:12px;color:#9ca3af;">
                &copy; ${year} GrantTrail &mdash; Questions? Contact
                <a href="mailto:support@granttrail.ca" style="color:#1a6b3c;text-decoration:none;">support@granttrail.ca</a>.
              </p>
            </td>
          </tr>
  ...
  await sendEmail({
    to: opts.to,
    subject: 'Finish setting up your GrantTrail listing',
    html,
  });
}
