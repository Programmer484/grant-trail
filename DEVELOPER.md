...
- These are NOT interchangeable. See the [Architecture doc](ARCHITECTURE.md) for the full distinction.
...
For full column-level detail see [`DATABASE.md`](DATABASE.md).
...
The full architecture is documented in [`ARCHITECTURE.md`](ARCHITECTURE.md). Key topics it covers:
...
In the render, a confirmation prompt is displayed when the item is armed:
...
{deletingId === att.id ? (
  ...
  <button className="ga-delete-btn" onClick={() => handleDelete(att)}>
    Delete
  </button>
)}
```
...
**Fix:** Check that `frontend/.env.local` exists and contains both `VITE_SUPABASE_URL` and `VITE_SUPABASE_KEY`. Restart `npm start` after creating or modifying the file.
...
**Symptom:** An expense definitely has a receipt in the database, but the Grant Breakdown expenses table shows `—` instead of the receipt icon button.
