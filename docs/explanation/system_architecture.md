...
- [1. Multi-Tenancy](#1-multi-tenancy)
  ...
  - [1.6 Roles — Super Admin vs Tenant Admin](#16-roles--super-admin-vs-tenant-admin)
...
- [3. Two-Tier SaaS Model](#3-two-tier-saas-model)
  ...
  - [3.6 Super Admin Responsibilities](#36-super-admin-responsibilities)
...
### 1.6 Roles — Super Admin vs Tenant Admin
...
| `super_admin` | All tenants | Create/manage tenants, cross-tenant visibility |

Options (implemented as option 1):
- Add `'super_admin'` (referred to as platform admin) to the `role` CHECK constraint and create a separate `is_super_admin()` SECURITY DEFINER function
...
- Admin pages — a **Tenant Management** section for super admins
...
| Super admin UI (tenant management pages) | Large |
...
A super admin creates the tenant and generates an invite link for the first admin user. That admin then invites grantees. No open signup.
...
### 3.6 Super Admin Responsibilities

A super admin (a role beyond tenant admin — see [Section 1.6](#16-roles--super-admin-vs-tenant-admin)) manages both tiers:

| Task | How |
|------|-----|
| Create a managed tenant | Super admin UI: enter org name, set approval settings, generate first invite |
| Monitor self-service signups | Super admin UI: list of all tenants with type, creation date, user count |
...
| Super admin: tenant list + create managed tenant + disable tenant | Moderate-Large |
