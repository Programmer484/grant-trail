...
- **Variables:** `SUPABASE_PROJECT_REF`, `STRIPE_PRICE_BASIC`,
  `STRIPE_PRICE_FISCAL_AGENT`, `STRIPE_BILLING_PORTAL_CONFIGURATION_ID`, `APP_URL`,
  `VITE_SUPABASE_URL`, `VITE_SUPABASE_KEY`, `VITE_SENTRY_DSN`, `EMAIL_FROM`

`ci` carries **secrets only**: `STRIPE_SECRET_KEY_TEST`,
`STRIPE_PRICE_BASIC_TEST`, `STRIPE_PRICE_FISCAL_AGENT_TEST`.
