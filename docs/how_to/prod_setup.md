...
4. **Vercel Credentials (for GitHub Actions):** Go to your Vercel Account settings to create a token and gather your IDs, then add them as **Secrets** to your GitHub `production` environment:
   ...
   - **Name:** `VERCEL_ORG_ID` | **Value:** Find your Org/User ID under **Settings → General** (labeled **Your ID**, starts with `usr_...` or `team_...`) in the Vercel dashboard.
   - **Name:** `VERCEL_PROJECT_ID` | **Value:** Find your Project ID under **Settings → General** (labeled **Project ID**, starts with `prj_...`) in your Vercel project settings.
